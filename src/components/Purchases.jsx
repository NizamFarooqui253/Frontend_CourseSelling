...
//  console.log("Puchased Items:",purchase);
...
//       console.log("Token got it::",token);
...
//             `http://localhost:8000/user/purchased`,
...
//           setPurchase(response.data.viewpuchaseCousre);
//         } catch (error) {
...
function Purchases() {
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      try {
        const response = await axios.get(`${url}/user/purchased`, {
          headers: {
            Authorization: `Bearer ${token}`,
          }, 
          withCredentials: true,
        });
        setPurchases(response.data.viewpuchaseCousre || []);
      } catch (error) {
        console.log(error);
  ...
  const handleLogout = async () => {
    try {
      ...
      console.log("Error in logout", error);
      toast.error(error.response?.data?.message || "Error in logout in purchase section");
    }
  };
